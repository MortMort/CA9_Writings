...
% How to change font size: put 9pt, 10pt, 11pt, 12pt in the first position here:
\documentclass[12pt, onecolumn,a4paper,compsoc]{IEEEtran}
...
\usepackage[textsize=small, textwidth=24mm]{todonotes}


% Set margins - The IEEE margins are WAY too thin.. (The template mentions that it is
% specifically a bad idea to do this but we do it anyway. Maybe something breaks?)
\usepackage[inner=20mm, outer=20mm, bottom=25mm, top=25mm, 
			marginparwidth=25mm, 
			marginparsep=-5mm]{geometry}

% Set marginparwidth for todonotes (it doesn't change the margin width. It just lets the
% text box have more space in the marings)
...
% Add section numbers to bookmarks
...
% Note that the Computer Society format requires a larger sans serif 
...
%\Crefname{figure}{\textbf{Figure}}{\textbf{Figures}}
% Make equation references bold and write out "Eq. (x,x)"
...
% Include section number in figure number. Also resets figure number on each section.
\counterwithin{figure}{section}


% This float package was originall not uncommented here. It resulted in a lot of warnings though so i commented it out. Might cause problems but i haven't found any yet.
%\usepackage{float}
...
\author{ \IEEEauthorblockN{\textbf{CA9} \\\vspace{1mm} 	Martin Højlund Therkildsen } \\\vspace{4mm}
	\IEEEauthorblockA{\begin{normalsize} Technical Faculty of IT and Design \\
		Department of Control and Automation \\\vspace{0.5mm}
		Frederik Bajers Vej 7C \\
		9220 Aalborg East \\\vspace{0.5mm}
		Email: \href{mailto: mtherk21@student.aau.dk}{mtherk21@student.aau.dk}} \\\vspace{4mm}
	\textbf{Supervisors}\\
	Torben Knudsen \\\vspace{2mm}
	\textbf{Contacts at Vestas} \\
	Sune Baun Christensen \\
	Jesper Sandberg Thomsen\end{normalsize}
}
...
% make the title area
\maketitle
% As a general rule, do not put math, special symbols or citations in the abstracts
% no keywords
\newpage
...
%\input{Chapters/Modelling_something}
...
\clearpage
\newpage
\input{Chapters/TestLinearMod}

% TEST - REAL SYSTEM
% ---------------------------
\clearpage
\newpage
\input{Chapters/TestRealSys}
...
% The subfigure \label commands are set within each  command,
...
\clearpage
\input{Chapters/Documentation_tj02}
