...
\end{equation}
The linear model of the generator is obtained through a taylor expansion:
...
\begin{align}
	...
	& = -P_g \, \dfrac{1}{\omega^2}
\end{align}
The above derived generator model is referred to the low-speed generator side by replacing $ \omega $ with $ \omega_L $ where $ \omega_L = \left (\frac{N_r}{N_g} \right ) \omega $ and since the generator model is implemented assuming a stiff drivetrain then $ \omega_L = \Omega $. This yields the final linear generator model evaluated at the operating point $ (P_{g_o}, \Omega_o) $:
\begin{equation}
	\begin{split}
		T_g(P_g, \Omega) 	& \approx \left. \dfrac{1}{\dfrac{N_g}{N_r} \Omega} \right |_{P_{g_o},\Omega_o} (P_g - P_0) \\ 
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=0.95\linewidth]{Graphics/wtLinForeAftMotionModel.pdf}
...
\subsubsection{Pitch system dynamics} \label{sec:comp_pitch}
...
A low dampening factor $ \zeta $ is assumed as a start since the dampening from the blades is modelled in the \hyperref[sec:comp_aero_thrust]{\textbf{aerodynamic thrust model}} \cref{sec:comp_aero_thrust}. Thus the only contributors to dampening of the system are from the other mentioned forces.... \textbf{This is where i left off. I'm not done at all}
